...
//   - Legacy: 43-char base64url, i.e. 32 random bytes. The original
//     client-side token from before the short-code switch.
//   - Short: 8-char Crockford base32, generated by the new
//     admin / user "issue recovery code" flows. Lower entropy
//     (40 bits) but trivially typeable on mobile.
...
// Used to mint short recovery codes from both admin and user flows.
...
// replayed as a bearer credential. No salt: with the entropy floor
// of even the short codes (40 bits) there's nothing to harden
// against at this scale.
...
// retains it; the server stores sha256(token) in the tokens table.
// Multiple tokens per profile are allowed (one per device); a hash
// that matches ANY of a profile's tokens is an idempotent re-claim.
// If the name is taken but the token doesn't match any known token
// for that profile, return 409 so the client can switch to the
// "I have a code" flow and paste a real device token.
...
// Bearer-token middleware. Looks up the caller's profile by token
// hash through the tokens table, so any device the user has signed
// in on can authenticate.
...
// Reuses generateShortCode() (same alphabet, same length).
...
app.get('/admin', (_req, res) => {
  ...
  const profileRows = profiles
    .map((r) => `<tr><td>${escapeHtml(r.name)}</td><td>${escapeHtml(r.created)}</td><td>${escapeHtml(r.seen)}</td><td class="num">${r.authored}</td><td class="num">${r.plays}</td><td><button class="btn-add-token" data-pid="${r.id}" data-pname="${escapeHtml(r.name)}">Issue new device code</button></td></tr>`)
    .join('');
...
<script>
// Add-device-code wiring for the profiles table. Mints a new short
// (8-char) code for the profile and ADDs it to the tokens table.
// The user's existing devices keep working; the new code lets them
// sign in on an additional device. Surface the code once for the
// admin to copy and send out-of-band.
document.addEventListener('click', function (ev) {
  ...
  fetch('/admin/profiles/' + pid + '/add-token', { method: 'POST' })
    ...
    .then(function (res) {
      if (!res.ok) {
        btn.disabled = false;
        btn.textContent = 'Issue new device code';
      ...
      input.style.cssText = 'width:120px;font-family:monospace;font-size:13px;letter-spacing:1px;background:#1f1b2e;color:#ece8ff;border:1px solid #4a416c;border-radius:3px;padding:3px 5px;';
...
// table. The user's existing devices keep working, this just gives
// them another credential to use on a new device (or to hand to
// themselves after losing access on one device). Returns the new
// plaintext code; this is the only chance to see it before it's
// hashed for storage. Anonymous endpoint, matches the open-
// dashboard posture; gate this behind an admin secret before
// deploying to anywhere that matters.
...
// Same as the admin endpoint, but bearer-authenticated: the user
// generates a new device code for themselves. Used by the
// "Add new device" button in the start menu. The new code is added
// to the tokens table; the calling device's token stays valid.
